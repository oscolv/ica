...
\usepackage{geometry}
\geometry{margin=2.3cm,headsep=14pt,headheight=14pt}
\usepackage{fontspec}
...
% Tinta como color global (NO como opcion de fuente: fontspec Color= anularia
% todos los \color del documento — titulos, banners, cabeceras, colofon)
...
\hypersetup{colorlinks=true,linkcolor=primary,urlcolor=accent,citecolor=primary,linktoc=all,
  pdftitle={Guia de referencia ICA - Indice de Calidad del Agua},pdfauthor={ICA}}
% ---- Índice general -----------------------------------------------------
% Antes ocupaba dos páginas y la segunda quedaba vacía en tres cuartos. Se
% compacta a una: la parte como rótulo-cintillo, la sección como entrada
% fuerte con número colgado, y la subsección en gris con guía de puntos.
\titlecontents{part}[0em]
  {\addvspace{8pt}\color{muted}\footnotesize\sbseries}
  {\MakeUppercase{Parte \thecontentslabel}\enspace}{}
  {\hfill}[\vspace{1pt}{\color{line}\hrule height 0.6pt}\vspace{3pt}]
\titlecontents{section}[1.6em]
  {\addvspace{1.5pt}\sbseries\color{ink}}
  {\contentslabel{1.6em}}{\hspace*{-1.6em}}
  {\hfill\color{primary}\contentspage}
\titlecontents{subsection}[3.6em]
  {\small\color{muted}}
  {\contentslabel{2.2em}}{\hspace*{-2.2em}}
  {\titlerule*[6pt]{\color{line}.}\color{muted}\contentspage}

\DeclareCaptionFont{icasb}{\sbseries}
% textfont: los floats arrancan con \normalcolor (negro kernel); el texto del caption va en tinta
\captionsetup{font=small,labelfont={icasb,color=primary},textfont={color=ink}}
...
% ---- Disciplina de página ----------------------------------------------
% Nada de líneas sueltas: una viuda o un huérfano cuesta más que una página
% que cierra un poco antes. Con \raggedbottom ese aire sobrante se va al pie
% (donde no se nota) en vez de estirarse entre párrafos (donde sí).
\widowpenalty=10000
\clubpenalty=10000
\displaywidowpenalty=10000
\brokenpenalty=10000
\raggedbottom
% Un título nunca se queda solo al pie: exige sitio para el rótulo + 3 líneas.
% 8 líneas: varias secciones abren con un banner (tcolorbox), que es más alto
% que el párrafo que \Needspace mediría. Con 5 el rótulo de §15 se quedaba
% solo al pie y su banner saltaba de página.
\pretocmd{\section}{\Needspace*{10\baselineskip}}{}{}
\pretocmd{\subsection}{\Needspace*{4\baselineskip}}{}{}
\pretocmd{\subsubsection}{\Needspace*{3\baselineskip}}{}{}
% Columnas estrechas de tabla: en 3–5 cm la justificación abre ríos y parte
% palabras («incumplim-ientos»). En medidas cortas, bandera a la derecha.
...
% Alineadas a la izquierda (lee más a libro que centrado) y con el sumario de
% la parte: eran cinco páginas casi vacías y ahora orientan al lector.
\titleclass{\part}{page}
\assignpagestyle{\part}{empty}
% Sumario de la parte: mismo nivel «section», pero con prefijo «p» para no
% pisar el formato del índice general.
\titlecontents{psection}[2.1em]
  {\addvspace{5pt}\sbseries\color{ink}}
  {\contentslabel{2.1em}}{\hspace*{-2.1em}}
  {\titlerule*[6pt]{\color{line}.}\color{primary}\contentspage}
% \normalfont\normalsize: el after-code de \titleformat hereda el cuerpo del
% título (30 pt ExtraBold); sin reponer la fuente, el sumario sale gigante.
\newcommand{\partsumario}{%
  \vspace{26pt}\par\normalfont\normalsize\color{ink}%
  \begin{minipage}{0.66\textwidth}
    {\color{muted}\footnotesize\sbseries EN ESTA PARTE\par}
    \vspace{3pt}{\color{line}\hrule height 0.6pt}\vspace{7pt}
    \printcontents[parts]{p}{1}{\setcounter{tocdepth}{1}}
  \end{minipage}\par}
\titleformat{\part}[display]
  ...
  {\begin{tikzpicture}[remember picture,overlay]
   ...
   \end{tikzpicture}%
   \vspace*{3.2cm}
   {\color{muted}\large\sbseries PARTE}\\[10pt]
   % Sin tracking, «II» y «III» se leen como marcas de conteo, no como
   % números romanos. El espaciado los separa en cifras legibles.
   {\color{primary}\fontsize{76}{76}\selectfont\ebseries
    \addfontfeature{LetterSpace=14}\thepart}}
  {16pt}
  {\color{ink}\fontsize{30}{37}\selectfont\ebseries\RaggedRight}
  [{\vspace{12pt}{\color{accent}\rule{0.30\textwidth}{2.5pt}}\startcontents[parts]\partsumario}]
...
% \firstleftmark, no \leftmark: el folio debe nombrar la sección en la que
% entras al abrir la página, no la última que empieza al pie (que aún no has
% leído). Con \leftmark, la página de «Qué es ICA» se anunciaba como «Los tres
% factores».
\rhead{\small\color{muted}\firstleftmark}
...
% colupper: tcolorbox pone el cuerpo en negro por defecto; lo fijamos a la tinta
% `lines before break`: si un banner no puede dejar 4 líneas antes del corte,
% pasa entero a la página siguiente. Evita los jirones de 2 líneas colgando en
% la cabecera de la página, que era el peor defecto de maquetación del PDF.
\tcbset{colupper=ink,lines before break=4,before skip=8pt,after skip=10pt}
...
% Escala 0–100 del índice: la portada tenía un hueco muerto entre la tarjeta de
% entrada y el pie. En vez de rellenarlo con adorno, se pone el objeto central
% del documento —el semáforo de categorías— a tamaño de portada.
% Unidad x = 1 punto del índice, así los tramos salen a escala real.
\newcommand{\escalaica}{%
\begin{tikzpicture}[x=\linewidth/100,y=1pt]
  \begin{scope}
    \clip[rounded corners=6pt] (0,0) rectangle (100,24);
    \fill[catmala]       (0,0) rectangle  (45,24);
    \fill[catmarginal]  (45,0) rectangle  (65,24);
    \fill[catregular]   (65,0) rectangle  (80,24);
    \fill[catbuena]     (80,0) rectangle  (95,24);
    \fill[catexcelente] (95,0) rectangle (100,24);
  \end{scope}
  % cortes en blanco: separan los tramos sin añadir una línea de color más
  \foreach \x in {45,65,80,95} \draw[white,line width=1.1pt] (\x,0) -- (\x,24);
  % Extremos arriba, cortes abajo: puestos en la misma línea, «95» y «100»
  % chocaban (solo hay 5 puntos de índice entre ellos).
  \node[anchor=south west,font=\scriptsize\sbseries,text=muted] at   (0,27) {0};
  \node[anchor=south east,font=\scriptsize\sbseries,text=muted] at (100,27) {100};
  \foreach \x in {45,65,80,95}
    \node[anchor=north,font=\scriptsize\sbseries,text=muted] at (\x,-4) {\x};
\end{tikzpicture}}
...
\vspace{1.5cm}
% 36 pt, no 38: a 38 el título se salía 17.6 pt de la caja y rompía la
% alineación derecha con la tarjeta de entrada que va debajo.
{\noindent\fontsize{36}{43}\selectfont\ebseries Índice de Calidad del Agua\par}
\vspace{10pt}
...
\vspace{1.5cm}
{\noindent\color{muted}\footnotesize\sbseries LA ESCALA DEL ÍNDICE\par}
\vspace{16pt}
\noindent\escalaica\par
% Reparto uniforme, no centrado por tramo: los dos últimos tramos (15 y 5
% puntos) son más estrechos que sus propias pastillas y se solapaban.
\vspace{20pt}
\noindent\hbox to \linewidth{%
  \catchip{catmala}{Mala}\hfill
  \catchip[catmarginalink]{catmarginal}{Marginal}\hfill
  \catchip[catregularink]{catregular}{Regular}\hfill
  \catchip[catbuenaink]{catbuena}{Buena}\hfill
  \catchip{catexcelente}{Excelente}}
...
\includegraphics[width=0.64\textwidth]{manual_assets/f3_curve.pdf}
...
\section{Categorías}
% 1.45 (antes 1.7): las pastillas siguen holgadas y la tabla recupera las
% líneas que hacían falta para que el banner «Cuidado con las fronteras» —que
% comenta justo esta tabla— no quedara huérfano en la página siguiente.
...
\shot{manual_assets/site_resultados.png}{0.72\textwidth}
...
\section{Referencias}
% Sangría francesa: en una lista bibliográfica el ojo baja por el apellido del
% autor, así que la primera línea manda y las de continuación se retiran.
\begin{itemize}[leftmargin=1.4em,label={},itemindent=-1.4em,itemsep=4pt]
